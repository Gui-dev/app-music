	...
	return (
		<View className="flex-row justify-around border-t border-border px-4 pb-4 pt-3">
			...
			>
				...
				/>
				<Text
					className={`mt-1 text-xs ${screen === 'Player' ? 'text-primary' : 'text-text-secondary'}`}
				>
					Player
				</Text>
			</TouchableOpacity>
			...
			>
				...
				/>
				<Text
					className={`mt-1 text-xs ${screen === 'Search' ? 'text-primary' : 'text-text-secondary'}`}
				>
					Search
				</Text>
			</TouchableOpacity>
			<TouchableOpacity
				onPress={() => setScreen('Biblioteca')}
				className="items-center"
			>
				<Ionicons
